...
WHERE ADMISSIONNUM IN (SELECT ADMISSIONNUM
                       FROM STAYIN
...
SELECT Doctor.EmployeeID AS DoctorID,
       Doctor.GraduatedFrom,
       CASE
           WHEN (SELECT COUNT(DISTINCT Examine.AdmissionNUM) -- If the admission_num dup, then only count once
                 FROM Examine
                 WHERE Examine.DoctorID = Doctor.EmployeeID) >
                10 -- If more than 10 distinct admissions, the doctor is overloaded
...
FROM Doctor,
     Employee
WHERE Doctor.EmployeeID IN (SELECT DoctorID
                            FROM DoctorsLoad
...
SELECT DISTINCT Doctor.EmployeeID                             AS DoctorID,
...
FROM Examine,
     ...
     CriticalCases,
     DoctorsLoad
WHERE Examine.DoctorID = Doctor.EmployeeID          -- Match Doctor ID
  ...
  AND Doctor.EmployeeID IN (SELECT DoctorID
                            FROM DoctorsLoad
                            WHERE load = 'Underloaded' -- Filter for underloaded doctors
)
  AND Patient.PatientSSN IN (SELECT Patient_SSN
...
CREATE OR REPLACE TRIGGER InsurancePayment
    ...
    FOR EACH ROW
BEGIN
    IF :NEW.TOTALPAYMENT != :OLD.TOTALPAYMENT THEN
...
BEGIN
    ...
    IF (SupervisorRank != :new.EMPRANK + 1) THEN
        RAISE_APPLICATION_ERROR(-20004,
                                'Incorrect supervisor rank. Supervisor ranks must be one higher than the employees ' ||
...
--Q5 ----------------------------------------------------------------------------------------
/*5. When a patient is admitted to an Emergency Room (a room with an Emergency service)
...
BEGIN
    ...
    IF (:NEW.AdmissionNum IN (possibleERs)) THEN
        :NEW.FutureVisitDate := :NEW.AdmissionDate + INTERVAL '2' MONTH;
    END IF;
end;
